...
public class AnnounceTest extends AbstractEntityTest
{
    ...
    void serializationWithReferences() throws URISyntaxException, IOException, ActivityPubException
    {
        ...
        String expectedJson = this.readResource("announce/announce1.json");
        assertEquals(expectedJson, this.serializer.serialize(announce));
    }
    ...
    {
        Announce expectedAnnounce = new Announce()
                .setActor(
                        new ActivityPubObjectReference<AbstractActor>()
                                .setLink(new URI("https://social.example/alyssa/"))
                )
        ...
        String json = this.readResource("announce/announce1.json");
        assertEquals(expectedAnnounce, this.parser.parse(json));
    }
}
